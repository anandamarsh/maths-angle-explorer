...
/** Loud clicky keypad key press — plays 3 stacked typewriter-style clicks at max gain. */
export function playKeyClick() {
  // Play it 3 times stacked so it can't be missed
  for (let rep = 0; rep < 3; rep++) {
    ...
    flt.type = "bandpass";
    flt.frequency.value = 3200 + rep * 400;
    flt.Q.value = 1.2;
    const gain = c.createGain();
